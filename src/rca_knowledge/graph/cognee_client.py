"""Thin wrapper around Cognee for the POC.

Cognee provides cognify pipelines, vector + graph storage, and a search API.
We pre-process documents through our own semiconductor-focused extractor,
then hand the structured triples back to Cognee as ingestible text so the
graph contains both raw evidence and our distilled causal claims.
"""
...
class CogneeClient:
    ...
    async def add_text(self, text: str, *, dataset: str = "rca", node_set: list[str] | None = None) -> None:
        """Add a single text blob into the cognee dataset."""
        await self.setup()
        await cognee.add(text, dataset_name=dataset, node_set=node_set)

    async def add_documents(self, paths: Iterable[Path], *, dataset: str = "rca") -> None:
        await self.setup()
        for p in paths:
            await cognee.add(str(p), dataset_name=dataset)

    async def cognify(self, *, dataset: str = "rca") -> None:
        """Run cognee's pipeline (chunk → extract → embed → graph)."""
        await self.setup()
    ...
    async def search(
        ...
        *,
        ...
        """Search the graph. Returns whatever cognee gives us — caller decides how to render."""
        await self.setup()
    ...
    async def prune(self) -> None:
        """Reset both data and system stores. Destructive."""
        await self.setup()
