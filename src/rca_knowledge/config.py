...
class Settings:
    ...
    llm_model: str             # default model for both extraction and reasoning
    llm_api_key: str           # the active key (matches llm_provider)

    extraction_model: str      # falls back to llm_model
    reasoning_model: str       # falls back to llm_model
    ...
    def export_to_cognee_env(self) -> None:
        """Cognee reads its config from os.environ; mirror our settings into it."""
        ...
        # Cognee 1.0 turns multi-user ACL on by default, which is incompatible
        # with the networkx graph backend ("the selected graph dataset to
        # database handler does not work with the configured graph database
        # provider"). POC is single-user — disable.
        os.environ.setdefault("ENABLE_BACKEND_ACCESS_CONTROL", "false")
        # Cognee/LiteLLM also read these directly:
...
settings = load_settings() if _can_load() else None
"""Module-level singleton; None until at least one provider key is present.

Scripts call `load_settings()` directly so they can fail fast with a clear message.
"""
